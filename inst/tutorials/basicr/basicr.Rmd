...
The code above also illustrates that value can alos be assgned using the `=` sign. However, from now on and for the sake of consistency, we will always use `<-` for variable assignments.
...
vec1[-1]
```
The first line extracts the first element of the vectore while the second line all **but** the first element of the vector.
...
Using the vector `vec2` that has been created before to perform the following operations.
```{r}
vec2
```
...
2. Compute the sum of the average of the first five elment of  the vector 
...
We have used so far several `R` functions. If you are unsure abou what a particular function does you can get some help using either `help` or `?`. For instance,
...
Note that in the defintion of the  function `p=2` means that `p`  is equal to 2 by default. Therefore `pow(2, 2)` returns the same value as `pow(2)`.
...
Now complete the following code to create a function that calculates the effective annual interst rate, $i$, given the nominal annual interest rate (monthly compounding), $j$. The default value is $i=0.05$. The fomula is $j=(i/12)^{12} - 1$.
