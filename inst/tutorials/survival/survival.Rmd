...
In order to conduct Log-rank and Wilcoxon tests for the difference between two survival curves, you will need to use the `survdiff` function. Setting the argument `rho` equal to zero will specify a Log-rank test. 
...
Alternatively, setting the `rho` argument to 1 will specify the Wilcoxon test:
...
2. Apply the Wilcoxon test and test whether the survival curves for the two types of transplant of the `alloauto` data are the same. 
...
question("At a critical value of 5%, what can you conclude from the test results?",
         answer("The Wilcoxon test suggests significant difference, while the log-rank test does not.", message = "Both p-values are larger than 5% so there is no significant difference."),
         answer("The log-rank test suggests significant difference, while the Wilcoxon test does not.",message = "Both p-values are larger than 5% so there is no significant difference."),
